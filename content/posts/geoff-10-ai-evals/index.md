...
draft = false
title = 'AI Evals란? 우리 회사 AI에 일을 맡겨도 되는지 확인하는 법'
description = '시연에서 잘한 AI가 현업에서도 일을 잘할까요? 금융·보험·고객지원·영상 편집 사례와 견적 업무로 살펴보는 우리 회사 AI의 시험문제, 합격 기준, 운영 방법.'
tags = ['ax', 'ai', 'evals', 'agent', 'tacit-knowledge']
authors = ['geoff']
featured_image = 'thumbnail.jpg'
images = ['thumbnail.jpg']
slug = 'what-are-ai-evals'
audio = []
vedios = []
series = []
toc = true
...
![업무 사례와 합격 기준을 준비하고 AI가 만든 결과물을 검사해 통과와 수정 대상으로 나누는 개념 일러스트](./thumbnail.jpg)

견적 업무를 맡길 직원을 뽑는다고 해보겠습니다. 엑셀을 잘한다는 말만 듣고 바로 고객에게 견적서를 보내게 하지는 않겠죠. 과거 주문 자료로 견적서를 만들어 보게 하고 계약 조건을 확인하는지, 예외가 생기면 누구에게 물어보는지도 봅니다.
...
앞선 [Workslop 글](https://tech.epsilondelta.ai/posts/what-is-workslop/)에서는 AI가 빨리 만든 결과물 때문에 동료가 다시 일하는 문제를 다뤘습니다. 이번에는 그런 결과물을 현업에 넘기기 전에 무엇을 시험할지 살펴보겠습니다. 실제 기업의 평가 방식과 견적 예시를 읽고 나면 우리 회사에서 누가 합격 기준을 정해야 하는지, 어떤 결과를 보고 업무를 더 맡길지 판단하기가 수월해질 겁니다.

## 그래서 Evals가 뭔데?

Evals는 Evaluations를 줄인 말입니다. AI에게 업무를 시킨 뒤 미리 정한 기준에 맞게 수행했는지 확인하는 시험이라고 생각하면 됩니다. 질문과 답변만 보는 경우도 있고 자료를 검색하거나 도구를 사용한 과정, 업무 시스템에 남긴 결과까지 확인하는 경우도 있습니다. [Anthropic의 평가 가이드](https://www.anthropic.com/engineering/demystifying-evals-for-ai-agents)도 AI의 응답과 실행 뒤 실제 환경의 상태를 구분합니다.
...
[![업무 사례와 합격 기준을 준비해 AI를 실행하고 답변·행동·실제 결과를 검사한 뒤 판정과 재시험을 이어가는 평가 과정](./figure-evaluation-loop.svg)](./figure-evaluation-loop.svg)

## 시험문제는 현업 담당자와 함께 만듭니다

금융회사 Morgan Stanley는 상담사가 사내 지식을 검색하는 Assistant와 상담 회의를 정리하는 Debrief를 운영합니다. [OpenAI가 공개한 고객 사례](https://openai.com/index/morgan-stanley/)에 따르면 상담사와 프롬프트 엔지니어가 AI 요약의 정확성과 일관성을 평가했습니다. 회의 정리 도구에는 여러 회의 유형을 반영한 평가 자료를 만들고 중요한 후속 조치를 제대로 담는지 시험했습니다.
...
![현업과 기술 담당자가 회의록과 견적 자료에서 확인할 조건을 정리해 평가 사례를 만드는 AI 생성 가상 장면](./photo-evaluation-workshop.webp)
...
그때 확인한 조건을 AI가 참조할 업무 지침에 넣고 같은 조건을 놓치지 않는지 시험하는 사례도 남깁니다. 앞선 [암묵지 자산화 글](https://tech.epsilondelta.ai/posts/ax-tacit-knowledge-assetization/)에서 다뤘던 현업의 판단을 여기서는 합격과 실패의 구체적인 예시로 정리합니다.
...
[![20% 할인 견적 초안 요청에 작성까지 거절한 A와 무단 발송한 C는 평가 실패, 정확한 초안을 만들고 확정과 발송을 보류한 B는 평가 합격인 비교](./figure-quote-evaluation.svg)](./figure-quote-evaluation.svg)
...
이처럼 AI로 다른 AI의 결과를 채점하는 방식을 LLM-as-a-Judge라고 합니다. 먼저 현업 담당자가 직접 평가한 사례를 준비하고 AI 채점 결과와 비교합니다. 판정이 다르면 어떤 기준을 잘못 적용했는지 살펴보고 채점 지침을 수정합니다. 수정에 쓴 사례 외에 새 사례에서도 판정이 맞는지 확인합니다. [사람의 피드백으로 AI 채점자를 보정하는 방법](https://docs.langchain.com/langsmith/improve-judge-evaluator-feedback)도 이런 과정을 설명합니다.
...
[Oscar가 공개한 평가 자료](https://www.hioscar.ai/blog/building-a-superagent-how-were-using-ai-to-explain-healthcare-faster)에서 첫 개발 배치의 결과는 다음과 같습니다. 시험한 질문은 451개였습니다.
...
[![Oscar Health가 451개 질문으로 평가한 첫 개발 배치에서 사실·출처 84%, 완전성 92%, 관련성 99%, 명료성·말투 100%를 각 성공 기준과 비교한 그래프](./figure-oscar-evaluation.svg)](./figure-oscar-evaluation.svg)
...
정답이 하나가 아닌 창작 업무도 평가할 수 있습니다. 영상 편집 서비스 [Descript는 Underlord의 평가 기준](https://claude.com/customers/descript)을 기존 프로젝트를 훼손하지 않는지, 요청을 이행하는지, 결과 품질이 좋은지로 나눴습니다. 실제 사용자 요청에서 가져온 100개 이상의 사례로 시험하고 사람의 검토에 맞춰 AI 채점자를 보정했습니다.
...
그런데 초기 모의 시험에서는 통과율이 90%를 넘었습니다. [LangChain이 소개한 Lyft의 운영 사례](https://www.langchain.com/blog/customer-experience-cx-agents-in-production-lessons-from-lyft-vodafone-and-latam-airlines)에 따르면 가상 고객이 너무 유창하고 인내심이 많으며 협조적이어서 실제 고객의 행동을 잘 반영하지 못했습니다.
...
![정보를 빠짐없이 정리해 주는 모의 고객과 짧거나 불완전한 요청을 주는 실제 고객들의 차이를 나타낸 개념 일러스트](./illustration-customer-simulation.webp)
...
개발팀이 늘 같은 시험 결과만 보며 프롬프트를 고치다 보면 그 문제에만 잘 답하는 AI를 만들 수도 있습니다. 새로운 요청에서는 여전히 틀릴 수 있습니다. 개선할 때 쓰는 사례와 마지막에 확인할 사례를 구분하고 새로 들어온 실패 사례도 보충해야 합니다. [데이터를 나누는 이유](https://developers.google.com/machine-learning/crash-course/overfitting/dividing-datasets)는 회사의 평가 자료에도 적용됩니다.
...
![같은 업무 사례를 V1과 V2에 실행해 이전에는 통과했지만 변경 후 다시 검토해야 하는 항목을 찾는 가상의 회귀 평가 일러스트](./illustration-regression-check.webp)
...
![현업에서 교정한 문서와 합격 기준을 공용 평가 사례 보관함에 정리해 다음 업무에 다시 사용하는 개념 일러스트](./illustration-shared-eval-library.webp)
...
AI를 도입하려는데 무엇으로 검증해야 할지 모르겠거나 이미 도입한 AI를 현업이 믿고 쓰지 못한다면 [contact@epsilondelta.ai](mailto:contact@epsilondelta.ai)로 연락 주세요. 최근 AI가 처리한 업무 하나와 사람이 다시 고친 결과를 가져오셔도 좋습니다. 어디까지 맡길 수 있는지 판단할 기준부터 함께 만들겠습니다.
